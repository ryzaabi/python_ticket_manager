...
class TicketManagerApp:
    ...
    def book_event(self, event_tree):
        ...
        def submit_booking():
            ...
            success_message = self.manager.book_tickets(event_id, int(num_tickets))
            messagebox.showinfo("Success", success_message)
            booking_window.destroy()

        # Open a modal for ticket input
        booking_window = tk.Toplevel(self.content_frame)
        booking_window.title("Book Tickets")
        booking_window.geometry("300x200")
        booking_window.configure(bg="white")
        booking_window.grab_set()  # Make it modal
        booking_window.resizable(False, False)  # Disable resize
        booking_window.attributes("-toolwindow", True)
